...
def check(_id):
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        result = cursor.execute('SELECT name, keys FROM `fofa` where id=(?)', (1,))
        for row in result:
            return row[0], row[1]
...
class Cmsscanner(object):
    ...
    def get_info(self):
        ...
        try:
            ...
            r = session.get(url=self.target, headers=agent,
    ...
    def run(self):
        try:
            ...
            #list index out of range 因为数据库中的id有缺失
...
class WhatCms:
    ...
    def request_url(url):
        try:
            ...
            r = requests.get(url=url, headers=requests_headers(),timeout=request_timeout,verify=True, proxies = web_proxy)
            r.encoding = 'utf-8'
    ...
    def set_info(self,finger_id= "nothing",cms_name= "Not Found",path= "nothing",match_pattern= "nothing",options="nothing",hit= "nothing"):
        self.lock.acquire()
        self.is_finish = True
    ...
    def find_cms_with_file(self):
        ...
        while True:
            ...
            if ( (options == "md5" and match_pattern == getMD5(response_content)) or 
                 (options == "keyword" and  match_pattern.lower() in response_html.lower())):
                self.set_info(finger_id,cms_name,path,match_pattern,options,hit)
                break
