    ...
    for ext in double_exts:
        if ext in domain:
            if len(domain.split('.')) > 3:
                ...
                domain_new = "%s.%s.%s" % (domain_split[-3], domain_split[-2],
                                           domain_split[-1])
                # print "exact",domain
...
excludeext = [
    '.png', '.ico', '.gif', '.svg', '.jpeg', 'js', 'css', 'xml', 'txt'
]
...
def getPageLinks(url):
    ...
    try:
        ...
        try:
            for tag in tags:
                for to in tos:
                    link1 = re.findall(r'<%s.*?%s="(.*?)"' % (tag, to),
                                       str(content))
                    link2 = re.findall(r'<%s.*?%s=\'(.*?)\'' % (tag, to),
                                       str(content))
                    for i in link1:
                        links.append(i)
...
def update_mysql(conn, url, filename, md5):
    try:
        ...
        if exist_md5:
            url = exist_md5[2] + ',' + url
            hint = exist_md5[3] + 1
            sql = "update file_md5 set url = '%s',hint='%s' where md5 = '%s'" % (
                url, hint, md5)
        else:
            sql = "INSERT INTO  file_md5 (url,filename,md5,hint) VALUES ('%s','%s','%s','%s')" % (
                url, filename, md5, 1)
        print sql
        cur.execute(sql)
        conn.commit()
...
conn = mysql.connector.connect(user=mysql_config['username'],
            ...
            if ext in suburl[-4:]:
                ...
                if re.findall(r'/', suburl):
                    if re.findall(r':', suburl):
                        ...
                        true_url.append(urlprotocol + '://' + domain_url +
                                        '/' + suburl)
                else:
                    true_url.append(urlprotocol + '://' + domain_url + '/' +
                                    suburl)
